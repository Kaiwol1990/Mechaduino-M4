...
void samd51TC5::calcOVF()
{
  ...
  GCLK->PCHCTRL[TC5_GCLK_ID].bit.GEN = 0;
  GCLK->PCHCTRL[TC5_GCLK_ID].bit.CHEN = 1;

  // wait till the generator is active
  while (GCLK->PCHCTRL[TC5_GCLK_ID].bit.CHEN == 0)
    ;
  ...
  TC5->COUNT16.WAVE.reg = TC_WAVE_WAVEGEN_MFRQ; // Set TONE_TC mode as match frequency

  tmpReg |= prescalerConfigBits;
  TC5->COUNT16.CTRLA.reg |= tmpReg;
  ...
  TC5->COUNT16.INTENSET.bit.OVF = 1; // set overflow
