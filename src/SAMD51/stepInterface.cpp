...
void stepInterface::readsteps() {
  ...
  while (TC0->COUNT32.SYNCBUSY.bit.CTRLB);                // Wait for the CTRLB register write synchronization
  while (TC0->COUNT32.SYNCBUSY.bit.COUNT);                // Wait for the COUNT register read sychronization
...
void stepInterface::writesteps(int32_t steps_) {
  ...
  while (TC0->COUNT32.SYNCBUSY.bit.COUNT);                // Wait for the COUNT register writ sychronization
}
